...
export default function Index({ permissions, queryParams, tables }: IPermissionProps) {
    ...
    const handleSort = (column: string) => {
        ...
        setSort(newSort);
        setCurrentPage(1);
        fetchPermissions(1, perPage, searchValue, newSort);
    };

    const [columnsPermissions, setColumnsPermissions] = useState(getColumnsPermissions(sort, handleSort));

    useEffect(() => {
        setPermissionsData(permissions.data);
        setColumnsPermissions(getColumnsPermissions(sort, handleSort));
    ...
    const applyFilters = () => {
        setCurrentPage(1);
        fetchPermissionsWithFilters(1, perPage, searchValue, sort, filters);
    };
    ...
    const clearFilters = () => {
        ...
        setCurrentPage(1);
        fetchPermissionsWithFilters(1, perPage, searchValue, sort, emptyFilters);
    };

    const fetchPermissionsWithFilters = (
        page = 1,
        perPage = 10,
        search = "",
    ...
    const fetchPermissions = (page = 1, perPage = 10, search = "", sort = { column: "name", direction: "asc" as "asc" | "desc" }) => {
        fetchPermissionsWithFilters(page, perPage, search, sort, filters);
    };
